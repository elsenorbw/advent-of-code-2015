...
def when_do_we_enter_the_basement(instructions: str, current_floor=0) -> Optional[int]:
    """
    Find which instruction has us enter the basement, returning None if we never do
    """
    for this_step_number, this_step in enumerate(instructions, start=1):
        if ")" == this_step:
            current_floor -= 1
        elif "(" == this_step:
            current_floor += 1
            ...
            raise RuntimeError(
                f"no idea what to do with {this_step} at step #{this_step_number}"
            )

        # exit condition
        if -1 == current_floor:
            return this_step_number
    return None
...
if __name__ == "__main__":
    ...
    part1_answer = when_do_we_enter_the_basement(instructions)
    print(f"Part 2 answer is {part1_answer}")
